...
const Process = () => {
    ...
    return (
        ...
        {process.map((item) => (
        <div key={item.id} className="md:p-5 p-2 duration-300 transform bg-white border rounded shadow-sm hover:-translate-y-2">
          <div className="flex items-center justify-between mb-2">
            <p className="md:text-CardHeading text-CardHeading-sm font-bold leading-5">{item.title}</p>
            <p className="flex items-center justify-center w-8 h-8 font-bold rounded text-purple-400 bg-indigo-50">
            {item.para}
            </p>
          </div>
