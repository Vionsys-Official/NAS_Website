    ...
    return (
        <div className='md:py-4 py-2'>
            <motion.div
              ...
              initial="hidden"
              whileInView={"show"}
              viewport={{ once: false, amount: 0.8 }} className="max-w-xl md:mx-auto sm:text-center lg:max-w-2xl">
                <h1 className='md:text-Heading text-Heading-sm font-bold tracking-tight text-blue950 sm:text-4xl sm:leading-none leading-10 text-center'>Our Process</h1>
                <p className="md:text-SubHeading text-SubHeading-sm text-gray-800 md:text-lg md:py-4 py-2 md:px-0 px-1">We ensure seamless software development from start to finish, guaranteeing your success at every stage.</p>
            </motion.div>

            <div className="relative grid gap-8 row-gap-5 mb-8 md:row-gap-8 lg:grid-cols-3 sm:grid-cols-2 py-4 md:px-16 px-4">
        ...
        initial="hidden"
        whileInView={"show"}
        viewport={{ once: false, amount: 0.8 }} key={item.id} className="md:p-5 p-2 duration-300 transform bg-white border rounded shadow-sm hover:-translate-y-2">
          <div className="flex items-center justify-between mb-2">
            <p className="md:text-CardHeading text-CardHeading-sm font-bold leading-5">{item.title}</p>
